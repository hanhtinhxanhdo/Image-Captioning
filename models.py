...
class EncoderCNN_VGG19(nn.Module):
    def __init__(self, embed_size):
        super(EncoderCNN_VGG19, self).__init__()
        ...
        modules = list(vgg19.features.children())
        self.vgg19 = nn.Sequential(*modules)
        
        # Replace the classifier with a custom linear layer
        self.embed = nn.Linear(512 * 7 * 7, embed_size)

    def forward(self, images):
        # Forward pass through VGG19 layers
        features = self.vgg19(images)
        
        # Flatten the output
        features = features.view(features.size(0), -1)
        
        # Linear layer to obtain the final image features
        features = self.embed(features)        
        return features
